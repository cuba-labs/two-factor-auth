...
public class TwoFactorLoginPasswordAuthenticationProvider extends AbstractAuthenticationProvider implements Ordered {
    ...
    private boolean check2fa(String login, Integer authKey, Locale credentialsLocale) {
        ...
        if (user.getTotpSecret() == null) {
            return true; //users with 2FA disabled can log in without auth code
        }
        //Now we're doing 2FA
        if (authKey == null) {
            throw new BadCredentialsException("You must provide two factor auth key to proceed");
        }
        ...
        try {
            ...
            throw new BadCredentialsException("Incorrect two factor auth key", e);
        }
    }
